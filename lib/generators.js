...
export async function generateForClass(classId) {
  ...
  const [cls] = await sbGet(`classes?id=eq.${classId}&select=id,type,class_date,instructor,pieces(id)`);
  ...
  for (const t of tasks) {
    const dueIso  = addDaysIso(cls.class_date, t.offset_days || 0);
    ...
    const baseRow = {
      ...
      phase:              t.phase,
      batch_key:          batchKey,
      assignee:           t.default_owner || cls.instructor || null,
...
export async function materializeGoalTasksForWeek(weekKey) {
  ...
  const [goalTasks, specialTasks] = await Promise.all([
    sbGet(`goal_tasks?deadline=gte.${weekKey}&deadline=lte.${weekEnd}&select=id,title,owner,deadline,status,notes,goal_id,goals(title)`),
    sbGet(`special_tasks?deadline=gte.${weekKey}&deadline=lte.${weekEnd}&select=id,title,scope,staff_id,deadline,status,week_key`),
  ]);
