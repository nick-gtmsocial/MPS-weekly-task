...
// row per task. Kiln-phase tasks (bisque, glaze-fire) only emit when the
// class has student_count > 0 — empty/no-show classes don't waste kiln
// slots. Kiln tasks snap to the next team-scheduled fire day on/after
// class_date+offset_days; non-kiln tasks land on the offset day directly.
//
// Non-destructive: uses INSERT ... ON CONFLICT DO NOTHING against the
// (class_id, class_type_task_id) live unique index, so re-running adds
// missing rows but NEVER touches existing rows' assignee/status — work
// the team has already done is preserved.
//
// per-piece scope from class_type_tasks is treated the same as per-class
// here: emit one row regardless of pieces.length. The team batches "trim
// pieces" / "glaze pieces" as a single class-level task; per-student
// granularity wasn't useful in practice.
//
// Returns { inserted, skippedDueToCount, totalTasks, batchKeys }.
export async function generateForClass(classId) {
  if (!classId) throw new Error('classId required');

  const [cls] = await sbGet(
    `classes?id=eq.${classId}&deleted_at=is.null&select=id,type,class_date,instructor,student_count`
  );
  ...
  const types = await sbGet(
    `class_types?name=eq.${encodeURIComponent(cls.type || '')}&select=id,name,pieces_per_student,class_type_tasks(*)`
  );
  const ct = types[0];
  if (!ct) {
    return {
      inserted: 0, skippedDueToCount: 0, totalTasks: 0, batchKeys: [],
      reason: `no class_type matches "${cls.type}"`,
    };
  }
  const tasks = (ct.class_type_tasks || []).sort((a, b) => a.sort_idx - b.sort_idx);

  // Defensive assignee validation — weekly_tasks.assignee FKs to staff(id).
  // Per Nick (2026-05-06): the 'shared' pseudo-assignee is being phased out;
  // every task should land on a specific named staff member. If the seed
  // says default_owner='shared', skip it and fall through to the class
  // instructor. Existing rows already assigned to 'shared' are NOT rewritten
  // — only newly-emitted rows pick a real owner. The team will reassign
  // legacy shared rows manually.
  const staffRows = await sbGet('staff?select=id');
  const validStaff = new Set(staffRows.map(s => s.id));
  const safeAssignee = (id) =>
    (id && id !== 'shared' && validStaff.has(id)) ? id : null;
  const pickOwner = (defaultOwner, instructor) => {
    // Explicit "manager assigns each week" — do NOT fall back to the
    // instructor. (null / 'shared' both fall through to the instructor;
    // 'unassigned' is the only way a class task lands with no owner.)
    if (defaultOwner === 'unassigned') return null;
    const primary = (defaultOwner && defaultOwner !== 'shared') ? defaultOwner : null;
    return safeAssignee(primary || instructor);
  };

  // Live kiln load (other classes only) for the walk-forward fallback when
  // the team hasn't planned a fire on the schedule yet.
  ...
  const rows = [];
  let skippedDueToCount = 0;

  for (const t of tasks) {
    const isKiln = t.phase && KILN_FIRE_PHASES.includes(t.phase);

    // Hard gate: kiln tasks require students. No fire on a no-show.
    if (isKiln && studentCount <= 0) {
      skippedDueToCount++;
      continue;
    }
    ...
    rows.push({
      ...
      phase:              t.phase,
      batch_key:          batchKey,
      assignee:           pickOwner(t.default_owner, cls.instructor),
  ...
  if (!rows.length) {
    return { inserted: 0, skippedDueToCount, totalTasks: 0, batchKeys: [] };
  }

  // Insert-on-conflict-DO-NOTHING. Existing rows (matching the partial
  // unique index on (class_id, class_type_task_id) where source_kind='class')
  // are left untouched — assignees + done statuses preserved. New rows
  // (e.g. a kiln task that was previously gated out by student_count=0)
  // get inserted. NO DELETE.
  await sbPost('weekly_tasks', rows, {
    onConflict: 'ignore',
    returning:  'minimal',
  });
...
export async function generateRecurringForWeek(weekKey) {
  ...
  const finalRows = rows.map(t => ({
    ...
    phase:              null,
    batch_key:          null,
    // 'shared' is phased out — recurring chores need a real owner. If the
    // seed says 'shared', emit null (forces the manager to assign).
    assignee:           (t.default_owner && t.default_owner !== 'shared' && validStaff.has(t.default_owner)) ? t.default_owner : null,
    duration_minutes:   t.duration_minutes,
    status:             'todo',
    notes:              t.notes || null,
    updated_at:         new Date().toISOString(),
